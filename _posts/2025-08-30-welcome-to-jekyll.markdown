...
It's a coroutine because it has the `co_return` keyword inside.
Now, if we try to compile it, we get a error (with a nice message).
...
It tells the compiler if it should destroy the coroutine frame

Just a quick note about the term **coroutine frame**. The _state_ of a coroutine lives
on the heap. Where _state_ is essentially what the stack frame of a function would be.
...
We can understand what they mean immediately. In `initial_suspend()`, change its return
type to `std::suspend_never`. Now run it again. We get the output:
...
In other words, `std::suspend_never` gives the execution flow to the body of the coroutine.
...
The same is not entirely true to coroutines. Remember that coroutines do not have a stack frame,
...
Now, don't print, we **yield**. Meaning that we want the caller of the coroutine
to receive this value - much like when a function returns a value.
Also, note that we removed the co_return statement. We don't need it anymore: `f()` is still
a coroutine because it has the `co_yield` keyword.
...
the parameter of `yield_value()`. Lastly, with the return type we indicate if after the `co_yield`
the coroutine continues to execute (`std::suspend_never`) or if it gives control
to the caller (`std::suspend_always`).
...
Let's go through it step by step:
  ...
  - now, we are in `get()`.
    ...
    because the coroutine has never executed anything yet.
    - because line 28 exists, we resume the coroutine (in our case, we resume it for the first time, so
    in plain english it is more accurate to say we _start_ the coroutine). Now the flow of execution
    goes to the first instruction of the coroutine: line 34. This is the yield instruction, which
    will essentially call the `yield_value()` method in the promise type - initializing `out_`. Moreover,
    since `yield_value()` returns a `std::suspend_always`, the coroutine gets paused, and the flow of
    execution goes back to the caller (line 29). Now, when read from `out_`, the value is actually there.

Also, I will take this chance to show you the importance of the return type of `final_suspend()`.
Change all `std::suspend_always` with `std::suspend_never`. Now, when we call the coroutine in line 39,
it will:
1. immediately start executing
2. when it hits the `co_yield` is will go on to execute the next line
3. when it reaches the end of the scope in line 35 it will deallocate the coroutine frame in the heap

Essentially, it will behave like a function.
Now, in line 40 when we call `handle.get()` to fetch the value, we will get a **segmentation fault**.
...
we should not use `coro_handle_` anymore. Otherwise, _seg fault_.

If we keep the coroutine frame alive, by changing the return type of `final_suspend()` back to
...
In our first example, the coroutine had a _print_ a simple empty `co_return` statement,
...
And that's it. We used an `int`, but of course it can be any type.
...
all local variables it contains will be _destroyed_, much like when a function ends. Pay attention: only the state
of the instance of the promise type depends on the return value of `final_suspend`. The local variables of the coroutine
always get cleaned up at the end of the scope.

This is, more often than not, a technicality. But it is good to know. For example, now you know that even if you
have `std::suspend_always final_suspend() {...}`, you should not keep any pointers in your promise type
...
Compiling this gives us the error:

  ![coawait_compile_error](/assets/coawait_compile_error.png){: width="600" }

This one is a bit more cryptic. It indicates something is missing in the type `std::string` itself.
...
This is because the method we need to define in order to use `co_await` is slightly less nice than the others.
...
One, we want to be able to tell the compiler if it should suspend always or never. Two,
we want to receive a std::string.

Before we go any further, this is a bit annoying...
Why is _await_ not as simple as the other methods? Well, let's write it the way we want to and try to spot
the issue.
...
If it returns `true`, the coroutine will not pause - i.e it will consider the value it was awaiting for
...
- `await_resume`: this method is called when the coroutine wants to fetch the value it is awaiting for.
After it consumes the value, it keeps on executing.
- `await_suspend`: this method is only called if `await_ready` returns `false`. It's argument
is the handle to the coroutine. Its return type can be `void` or something else - this is very interesting
and powerful, we will discuss it shortly.
...
This is why in `put()` we first initialize `in_`, and only then resume (in our case, start for the first time)
the coroutine.
